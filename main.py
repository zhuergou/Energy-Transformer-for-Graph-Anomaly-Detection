...
def train(model, g, args, device, model2):
    ...
    for e in range(args.epoch):
        ...
        logits = model2(g_bak.to(device), features.to(device))
...
if __name__ == '__main__':
    ...
    if args.run == 1:
        if homo:
            ...
            model2 = GraphTransformerNet(in_feats, h_feats, num_classes, graph, num_heads, n_layers, layer_norm, batch_norm, residual, dropout, args.ffn, args).to(device)
        else:
            model = BWGNN_Hetero(in_feats, h_feats, num_classes, graph,  d=order)
            model2 = GraphTransformerNet(in_feats, h_feats, num_classes, graph, num_heads, n_layers, layer_norm, batch_norm, residual, dropout, args.ffn, args).to(device)
        train(model, graph, args, device, model2)
        ...
        for tt in range(args.run):
            if homo:
                ...
                model = BWGNN_Hetero(in_feats, h_feats, num_classes, graph,  d=order)
                model2 = GraphTransformerNet(in_feats, h_feats, num_classes, graph, num_heads, n_layers, layer_norm, batch_norm, residual, dropout, args.ffn, args).to(device)
            mf1, auc = train(model, graph, args, device, model2)
