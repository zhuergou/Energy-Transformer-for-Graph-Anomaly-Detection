...
def train(model, g, args, device, model2, subsample_flag):
    ...
    index = list(range(len(labels_all)))
    if dataset_name == 'amazon':
        index = list(range(3305, len(labels_all)))

    idx_train, idx_rest, y_train, y_rest = train_test_split(index, labels_all[index], stratify=labels_all[index],
                                                            train_size=args.train_ratio,
                                                            random_state=2, shuffle=True)
    idx_valid, idx_test, y_valid, y_test = train_test_split(idx_rest, y_rest, stratify=y_rest,
                                                            test_size=0.67,
                                                            random_state=2, shuffle=True)
    train_mask_all = torch.zeros([len(labels_all)]).bool()
    val_mask_all = torch.zeros([len(labels_all)]).bool()
    test_mask_all = torch.zeros([len(labels_all)]).bool()

    train_mask_all[idx_train] = 1
    val_mask_all[idx_valid] = 1
    test_mask_all[idx_test] = 1
    print('train/dev/test samples: ', train_mask_all.sum().item(), val_mask_all.sum().item(), test_mask_all.sum().item())
    ...
    weight = (1-labels_all[train_mask_all]).sum().item() / labels_all[train_mask_all].sum().item()
    print('cross entropy weight: ', weight)
    time_start = time.time()

    for e in range(args.epoch):

        idx_train_sample = np.random.choice(idx_train, int(sample_ratio*len(idx_train)), replace=False)
        idx_valid_sample = np.random.choice(idx_valid, int(sample_ratio*len(idx_valid)), replace=False)
        idx_test_sample = np.random.choice(idx_test, int(sample_ratio*len(idx_test)), replace=False)
        

        sg = g.subgraph(list(idx_train_sample) + list(idx_valid_sample) + list(idx_test_sample))
        features = sg.ndata['feature']
        labels = sg.ndata['label']

        train_sample_size = len(list(idx_train_sample))
        valid_sample_size = len(list(idx_valid_sample))
        test_sample_size = len(list(idx_test_sample))

        tot_size = train_sample_size + valid_sample_size + test_sample_size

        train_mask = torch.zeros([tot_size]).bool()
        val_mask = torch.zeros([tot_size]).bool()
        test_mask = torch.zeros([tot_size]).bool()

        train_mask[:train_sample_size] = 1
        val_mask[train_sample_size: (train_sample_size + valid_sample_size)] = 1
        test_mask[(train_sample_size + valid_sample_size):] = 1
        ...
        model.train()
        model2.train()
        #logits = model(g_bak.to(device), features.to(device))
        #features_boost = model(features.to(device))
        logits = model2(sg.to(device), features.to(device), subsample_flag)
